...
def _python_list_depending_files(path: pathlib.Path, basedir: pathlib.Path) -> List[pathlib.Path]:
    ...
    try:
        ...
        if platform.uname().system == 'Windows':
            timeout = 10.0  # 1.0 sec causes timeout on CI using Windows
        else:
            timeout = 10.0
    ...
    node_deps_pairs = []
    for node in res_graph.graph:
        deps = [v for k, v in res_graph.graph.out_edges([node])]
        node_deps_pairs.append((node, deps))
